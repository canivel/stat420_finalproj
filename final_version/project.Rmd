...
We conducted ANOVA f-test on for each of these dummy predictor variable to get a feel on their effect on the response. The null hypothesis of each test would be that the mean of the response variable is the same for all levels of the dummy variable, by calculating the p-value and taking an $alpha=0.01$, we found that we could reject the null for the year, month and time dummy variables; the day of month and day of week variables on the other hand might not be siginificant.
