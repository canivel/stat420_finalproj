...
In the dataset, the continuous variables fare_amount, pickup_longitude, pickup_latitude, dropoff_longitude, dropoff_latitude can assume to be normally distributed. Since linear regression model coefficients are sensitive to outliers in the training data, we will use the boxplot rule to remove observations that has values greater than 1.5IQR away from the medium in each of these variables, which should help make our model generalize to unseen data that would follow a similar normal distribution.
...
** Looks like correlation is over control**
...
** the model with Step:  AIC=73279.59 was the best one for this amount of data, lets check the results**
...
** both models have the same p-value < 7.223e-07, and the comparision the model 1 perform a little better, but not to create a bigger difference **
...
```
** It appears that log transform hurts the performance of the model on the test set **

## Second model Polynomial
...
** Test log transform on response **
```{r}
m_int_log = lm(log(fare_amount) ~ .^2, train)
```

```{r}
# test rmse
sqrt(sum((test$fare_amount - exp(predict(m_int_log, test)))^2) / nrow(test))
```
** It appears that log transform hurts the performance of the model on the test set **
...
We added a different approach, xgboost.Rmd, using XGboost, that clear achive much better results with unseeing  data.
